...
def Arima(data, forecasting_length):
    model_arima = ARIMA(data['Returns'].dropna(), order=(3, 0, 3))
...
def Sarima(data, forecasting_length):
    # SARIMA (p,d,q) for non-seasonal, (P,D,Q,s) for seasonal
    model_sarima = SARIMAX(data['Returns'].dropna(), 
                           order=(1, 0, 1),  # (p,d,q)
                           seasonal_order=(1, 1, 1, 5))  # (P,D,Q,s) s = seasonality in timeframes eg. days
    fitted_sarima = model_sarima.fit()

    sarima_forecast = fitted_sarima.forecast(steps=forecasting_length)
    return sarima_forecast

def Egarch(data, forecasting_length):
    model_egarch = arch_model(data['Returns'].dropna(), vol='EGARCH', p=1, o=1, q=1)
...
def main():
    ...
    forecast = 5
    Garch = 0 # See Switch for dictionary
    Arima = 1
